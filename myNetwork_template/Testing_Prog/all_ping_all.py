...
def main(nbr_router_down):
    ...
    for k in range(number_ip):
        if k == nbr_router_down:
            pass
        else:
            net_rep = "myNetwork"
            router = read_output[k]
            test_router = router["name"]
            child.sendline('sudo ./connect_to.sh ' + net_rep + ' ' + test_router.upper())
            child.expect('\r\n')
            ...
            if idx == 1:
                ...
                for i in range(number_ip):
                    child.sendline('ping6 -c 1 -w 1 ' + ip_list[i])
                    child.expect('\r\n')
                    packet_loss_int = 666
